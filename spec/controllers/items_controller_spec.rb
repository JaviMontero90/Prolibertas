...
RSpec.describe ItemsController, type: :controller do
  ...
  json_attributes = FactoryGirl.attributes_for(:item).keys
  ...
  before :all do
    ...
    @resource = FactoryGirl.create(:item, person: @person)
    ...
    @first_page_resources = @person.items

    #Para el test de create
    @create_params = {person_id: @person.id.to_s}

    # Para el test de create y destroy
    @parameters = FactoryGirl.attributes_for(:item)

    # Para el test de update
    @update_params = FactoryGirl.attributes_for(:item_update)
  ...
  context "abilities" do 
    ...
    context "volunteer" do
      ...
      context "POST #create" do
        it "returns 403 HTTP status code" do
          post :create, person_id: @person.id.to_s, id: @resource.id.to_s, item: @create_params
          expect(response).to have_http_status :forbidden
        end
      end
